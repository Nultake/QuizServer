    ...
    public void run() {
        while(true) {
            ...
            if (input.startsWith("submit")){
                String request = input.split(":")[1];
                try {
                    BufferedWriter bw = new BufferedWriter(new FileWriter(new File("Submissions.txt"),true));
                    bw.append(request);
                    bw.newLine();
                    bw.flush();
                    bw.close();
            ...
            if (input.startsWith("getsubmitnumber")){
                Path path = Paths.get("Submissions.txt");
                try {
                    out.println(Files.lines(path).count());
                    out.flush();
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
            if(input.startsWith("getallsubmit")){
                try {
                    String respond = "";
                    BufferedReader br = new BufferedReader(new FileReader(new File("Submissions.txt")));
                    String line = null;
                    while((line = br.readLine()) != null){
                        respond+= line +",";
                    }
